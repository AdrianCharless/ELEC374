...
module control_unit_fsm (
    ...
    localparam [6:0]
        ...
        S_FETCH2 = 7'd3,
        S_HALT   = 7'd4,
        S_FETCH3 = 7'd78,   // decode state: IR is now valid, dispatch here
    ...
    always @(*) begin
        ...
        if (Stop) begin
            ...
            case (present_state)
                ...
                // FETCH2: latch IR from MDR, then go to FETCH3 for decode.
                // Do NOT dispatch here — IR register hasn't captured the new
                // instruction yet (it updates at the posedge that ends FETCH2).
                S_FETCH2: next_state = S_FETCH3;
 
                // FETCH3 (decode): IR now holds the current instruction.
                // Dispatch to the correct execute state based on opcode.
                S_FETCH3: begin
                    case (opcode)
