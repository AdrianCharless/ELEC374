...
// Following control sequence from Section 3.10 (Page 15) of CPU Phase 1 PDF
// Tests: ror R7, R0, R4
// ============================================================================
...
module ROR_tb;
    ...
    always @(Present_state) begin
        case (Present_state)
            ...
            Reg_load1a: begin
                Mdatain <= 32'h00000012;  // Load 18 into R0
                Read = 0; MDRin = 0;
                Read <= 1; MDRin <= 1;
                #15 Read <= 0; MDRin <= 0;
            end
            ...
            Reg_load1b: begin
                MDRout <= 1; R0in <= 1;
                #15 MDRout <= 0; R0in <= 0;  // R0 = 0x00000012
            end
            
            Reg_load2a: begin
                Mdatain <= 32'h00000004;  // Load 4 into R4 (rotation count)
                Read <= 1; MDRin <= 1;
                #15 Read <= 0; MDRin <= 0;
            end
            ...
            Reg_load2b: begin
                MDRout <= 1; R4in <= 1;
                #15 MDRout <= 0; R4in <= 0;  // R4 = 0x00000004
            end
            
            Reg_load3a: begin
                Mdatain <= 32'h00000055;  // Load arbitrary value into R7 (will be overwritten)
                Read <= 1; MDRin <= 1;
                #15 Read <= 0; MDRin <= 0;
            end
            ...
            Reg_load3b: begin
                MDRout <= 1; R7in <= 1;
                #15 MDRout <= 0; R7in <= 0;  // R7 = 0x00000055 (initial)
            end
            
            // Control Sequence for: ror R7, R0, R4
            T0: begin
            ...
            T1: begin
                ...
                Mdatain <= 32'h3B820000;  // Opcode for "ror R7, R0, R4"
                                          // 0011 1011 1000 0010 0000 ...
                                          // ROR=00111, Ra=0111, Rb=0000, Rc=0100
            end
            ...
            T5: begin
                ZLOout <= 1; R7in <= 1;
                // Result: R7 should contain R0 rotated right by R4 (4 positions)
                // R0 = 0x00000012 = ...00010010
                // ROR by 4 = 0x20000001 = 0010...00000001
            end
        endcase
    end
